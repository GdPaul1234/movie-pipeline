...
class ProcessMovieTest(unittest.TestCase):
    ...
    def test_log_progress_of_process_directory(self, mock_stdout):
        ...
        expected_push_action_json = json.dumps({
            "xy": 1,
            "push": {
                "actions": [
                    {"condition": "complete", "type": "run_event", "event_id": "process_movie", "params": {'file_path': str(self.video_path.absolute()), 'edl_ext': '__edl_ext_regex__'}, "enabled": True},
                    {"condition": "complete", "type": "run_event", "event_id": "process_movie", "params": {'file_path': str(self.serie_path.absolute()), 'edl_ext': '__edl_ext_regex__'}, "enabled": True}
                ]
            }
        })
        self.assertRegex(output, re.compile(re.escape(expected_push_action_json).replace('__edl_ext_regex__', r'\.pending_yml_\d+')))
